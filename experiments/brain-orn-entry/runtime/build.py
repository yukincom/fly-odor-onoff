"""Pinned solver with reversible OFF-only input gate for ten fixed ORNs."""
...
def build(root,out,jdk):
 ...
 for name in ('Connectome','PopulationIndex','LifConfig','LifNetwork'):
  ...
  if name=='LifNetwork':
   ...
   s=once(s,'        final float[] in = delayBuf[readSlot];','        final float[] in = delayBuf[readSlot];\n        if(arrivalGate!=null && arrivalGate.start(step,in,pending)) {\n            for(int i:arrivalGate.targets) if(in[i]!=0f && !active[i]) activate(i);\n        }')
   s=once(s,'            if (out[j] == 0f) pending[j]++;','            if(arrivalGate!=null) arrivalGate.edge((int)stepIndex,i,j,jump);\n            if (out[j] == 0f) pending[j]++;')
   (out/'intervention.patch').write_text(''.join(difflib.unified_diff(raw.decode().splitlines(True),s.splitlines(True),fromfile='upstream/LifNetwork.java',tofile='arrival-gated/LifNetwork.java')))
