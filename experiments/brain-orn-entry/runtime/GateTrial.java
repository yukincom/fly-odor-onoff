...
public final class GateTrial {
 public static void main(String[] args)throws Exception{
  ...
  ArrivalGate recorder=new ArrivalGate(c,cfg,targets,mode,out,name);net.arrivalGate=recorder;long start=System.nanoTime();
  try(recorder;PrintWriter w=new PrintWriter(Files.newBufferedWriter(out.resolve(name+"-populations.csv")))){
   ...
   for(int end=1;end<=finish;end++){
    ...
    if(end%50==0){net.endTick(50);Arrays.fill(previous,0);Arrays.fill(previousTargets,0);}
  ...
  Files.writeString(out.resolve(name+"-checks.json"),recorder.checks()+"\n");System.err.println("DONE "+name);
 }
}
