...
public final class PNTrial {
 public static void main(String[] args)throws Exception{
  ...
  int[] readout=Arrays.stream(pn).filter(i->Set.of(10176L,10208L,10390L,10561L).contains(c.bodyId[i])).toArray();if(readout.length!=4)throw new AssertionError("Four fixed olfactory PN required");int[] readPrev=new int[4];
  ...
  try(recorder;PrintWriter w=new PrintWriter(Files.newBufferedWriter(out.resolve(name+"-populations.csv")));PrintWriter trace=new PrintWriter(Files.newBufferedWriter(out.resolve(name+"-four-PN.csv")))){
   trace.print("end_ms,input_hz");for(int i:readout)trace.print(",cell_"+c.bodyId[i]);trace.println();
   ...
   for(int end=1;end<=finish;end++){
    ...
    trace.print(end+","+(on?odor*gain:0));for(int k=0;k<4;k++){int n=net.spikesThisTick(readout[k]);trace.print(","+(n-readPrev[k]));readPrev[k]=n;}trace.println();
    ...
    if(end%50==0){net.endTick(50);Arrays.fill(previous,0);Arrays.fill(previousTargets,0);Arrays.fill(pnPrev,0);Arrays.fill(readPrev,0);}
    if(end==1500||end==21500||end==24500){w.flush();System.err.printf("%s t=%.1f wall=%.1f%n",name,end/1000.,(System.nanoTime()-start)/1e9);}
   }
   if(trace.checkError())throw new AssertionError("readout write failed");
